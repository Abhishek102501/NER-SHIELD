...
class SecurityConfigTest {
    ...
    void unknownApiRouteRequiresAuthentication() throws Exception {
        // /api/simulations is not implemented yet — unlike /api/threats, /api/risk/zones,
        // /api/incidents and /api/alerts, it has no explicit permitAll() rule, so it must
        // still fall through to anyRequest().authenticated().
        mockMvc.perform(get("/api/simulations"))
                .andExpect(status().isUnauthorized())
                .andExpect(jsonPath("$.status").value(401))
                .andExpect(jsonPath("$.error").value("Unauthorized"))
                .andExpect(jsonPath("$.path").value("/api/simulations"))
